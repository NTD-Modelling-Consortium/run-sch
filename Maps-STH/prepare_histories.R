...
# for trichuris
#histories_joined$MDA_scheme[histories_joined$MDA_scheme == "IDA"] = "ALB+IVM" 
# rename drugs
#histories_joined$MDA_scheme[histories_joined$MDA_scheme %in% c("ALB","ALB/MBD")] = "Old Product B (SOC)" 
#histories_joined$MDA_scheme[histories_joined$MDA_scheme %in% c("ALB+IVM")] = "New Product A" 
...
save(iu_task_lookup, file="iu_task_lookup_sth.rds")

# save batch IDs to maps file
sth_distributions = iu_task_lookup %>% left_join(read.csv("sartorious_2021.csv") %>% filter(!p4==1))
write.csv(sth_distributions,file="sartorious_2021_withTaskID.csv", row.names = F)  
...
histories_joined_full[inds, colnames(histories_joined_full) %in% c("MDA_scheme")] = MBD_name
...
cat("Finished running prepare_histories.R \n")
