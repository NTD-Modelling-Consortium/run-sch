...
save(iu_task_lookup, file="iu_task_lookup_trichuris.rds")

# save batch IDs to maps file
sth_distributions = iu_task_lookup %>% left_join(read.csv("sartorious_2021.csv") %>% filter(!p4==1))
write.csv(sth_distributions,file="sartorious_2021_withTaskID_trichuris.csv", row.names = F)
...
for (id in 1:max(iu_task_lookup$TaskID)){
  ...
  iu_file<-file.path(inputs_path,paste0("IUs_MTP_trichuris_",id,".csv"))
  ...
  for (iu in ius[1,1]) { # all IUs in the same batch have the same treatment
    ...
    mda_path<-file.path(inputs_path,paste0("InputMDA_MTP_trichuris_",id,".xlsx"))
