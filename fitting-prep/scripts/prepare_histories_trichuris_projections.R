...
option_list <- list(
  make_option(c("-s", "--species"),
    type = "character",
    default = "trichuris",
    help = "Species to process (trichuris) [default=%default]"
  ),
  make_option(c("-i", "--id"),
    type = "integer",
    default = NULL,
...
projections_full[inds, colnames(projections_full) %in% c("MDA_scheme")] = IVM_name
...
save(proj_iu_task_lookup, file=file.path(kPathToMapsArtefacts, "proj_iu_task_lookup_trichuris.rds"))
...
for (id in batch_ids){
  ...
  for (iu in ius[,1]) {
    ...
    mda_path<-file.path(inputs_path,paste0("InputMDA_MTP_projections_trichuris_",iu,".xlsx"))
...
write.csv(table_iu_idx, file = file.path(kPathToMapsArtefacts, "table_iu_idx_trichuris.csv"), row.names = F)

cat("Finished running prepare_histories_trichuris_projections.R \n")
