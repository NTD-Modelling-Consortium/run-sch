...
if (species=="haematobium"){
  ...
  species_map_allyears = get(paste0("haematobium_maps"))
} else {
  load(paste0("../Maps-SCH/mansoni_maps.rds"))
  species_map_allyears = get(paste0("mansoni_maps"))
...
if (species == "haematobium"){ 
  table_iu_idx <- read.csv("../Maps-SCH/table_iu_idx_haematobium.csv")
} else {
  table_iu_idx <- read.csv("../Maps-SCH/table_iu_idx_mansoni.csv")
}
...
n_samples <- 500 # number of samples per iteration
num_sub_samples <- 200
...
mapped_prev_t1_medians = data.frame(species_map_allyears[[1]]$data[,1],median = apply(species_map_allyears[[1]]$data[,3:1002], 1, median))
colnames(mapped_prev_t1_medians) = c("IU_ID","mapped_prev_t1")
espen_prev_t1 = shape %>%
  dplyr::select(IU_ID,Shape_Leng,Shape_Area,geometry) %>%
  left_join(mapped_prev_t1_medians, by="IU_ID")
geomap_prev_t1 = st_as_sf(espen_prev_t1)

mapped_prev_t2_medians = data.frame(species_map_allyears[[2]]$data[,1],median = apply(species_map_allyears[[2]]$data[,3:1002], 1, median))
colnames(mapped_prev_t2_medians) = c("IU_ID","mapped_prev_t2")
espen_prev_t2 = shape %>%
  dplyr::select(IU_ID,Shape_Leng,Shape_Area,geometry) %>%
  left_join(mapped_prev_t2_medians, by="IU_ID")
geomap_prev_t2 = st_as_sf(espen_prev_t2)

mapped_prev_t3_medians = data.frame(species_map_allyears[[3]]$data[,1],median = apply(species_map_allyears[[3]]$data[,3:1002], 1, median)) 
colnames(mapped_prev_t3_medians) = c("IU_ID","mapped_prev_t3")
espen_prev_t3 = shape %>%
  dplyr::select(IU_ID,Shape_Leng,Shape_Area,geometry) %>%
  left_join(mapped_prev_t3_medians, by="IU_ID")
...
p_mapped_prev1 <- ggplot(geomap_prev_t1) +
  ...
  scale_fill_viridis(option=colour, direction = 1, limits=c(0,1), na.value="gray80") +
  theme_bw() + theme(legend.position="right") + 
  guides(fill=guide_colorbar(title="Median prevalence from \n geostatistical map")) +
  ggtitle(map_years[1]) + th
#p_mapped_prev1
...
fileName <- paste0(pathForPlots,"geomap_median_prevs_all_countries_",species_plot,".png")
...
if(plot_trajectories){
  ...
  for (year_ind in 1:length(prevalence_map)){
    ...
    if (length(which(!is.na(prevalence_map[[year_ind]]$data[j,]))) > 0){
      vioplot(prevalence_map[[year_ind]]$data[j,],add=T,col="red",at=year,wex=2)
    }
  }
