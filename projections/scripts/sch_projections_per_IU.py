...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run SCH nearterm projections for IUs in a batch ID",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Run projections for batch 10, haematobium species
  python sch_projections_per_IU.py --id=10 --species=haematobium
  
  # Run projections for batch 10, mansoni low burden, with 8 cores
  python sch_projections_per_IU.py --id=10 --species=mansoni_low_burden --num-cores=8
""",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="Batch/Task ID - processes all IUs with this TaskID. If unspecified, all batches will be processed.",
    )

    parser.add_argument(
        "--species",
        type=str,
        choices=["haematobium", "mansoni_low_burden", "mansoni_high_burden"],
        required=True,
        help="SCH species to run projections for. If unspecified, all species will be processed.",
    )

    parser.add_argument(
        "--num-cores", type=int, default=10, help="Number of cores to use (default: 10)"
    )
...
def run_projections_for_iu(iu, country, species, species_prefix, args):
    ...
    path_to_fitting_prep_artefacts = Path(
        os.getenv("PATH_TO_FITTING_PREP_ARTEFACTS", ".")
    )
    path_to_projections_prep_artefacts = Path(
        os.getenv("PATH_TO_PROJECTIONS_PREP_ARTEFACTS", ".")
    )
    path_to_model = Path(
        os.getenv(
            "STH_SCH_MODEL_DIR", "/ntdmc/sth-sch-amis-integration/model/ntd-model-sch"
        )
    )

    # Map species names to file paths
    species_dir_map = {
        "haematobium": "sch-haematobium",
        "mansoni_low_burden": "sch-mansoni",
        "mansoni_high_burden": "sch-mansoni",
    }
    species_dir = species_dir_map[species]

    # Coverage file path (from fitting-prep artefacts)
    coverage_file_name = (
        f"endgame_inputs/{species_dir}/InputMDA_MTP_projections_{iu}.xlsx"
    )
    ...
    rk_file_path = (
        path_to_projections_prep_artefacts
        / f"InputPars_MTP_{species}/InputPars_MTP_{iu}.csv"
    )
...
def main():
    """Main function to run SCH projections for a batch of IUs."""
    ...
    species_prefix_map = {
        "haematobium": "Hae_",
        "mansoni_low_burden": "Man_Low_",
        "mansoni_high_burden": "Man_High_",
    }
    ...
    if task_ids_to_process is None:
        ...
        print(
            f"Running SCH projections for ALL TaskIDs ({len(task_ids_to_process)} batches), species: {species}"
        )
        print(
            f"TaskIDs to process: {task_ids_to_process[:10]}{'...' if len(task_ids_to_process) > 10 else ''}"
        )
    else:
        ...
        print(f"Running SCH projections for TaskID {task_id}, species: {species}")
    ...
    for task_id in task_ids_to_process:
        ...
        if len(task_ids_to_process) > 1:
            print(
                f"TaskID {task_id}: ✓ {len(successful_ius)} successful, ✗ {len(failed_ius)} failed"
            )
    ...
    if len(task_ids_to_process) == 1:
        print(
            f"PROJECTION SUMMARY for TaskID {task_ids_to_process[0]}, species: {species}"
        )
    else:
